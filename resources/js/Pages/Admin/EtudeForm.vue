...
onMounted(() => {
    props.shifts.forEach((shift) => {
        let prix;
        props.Etude.prix_exams
            ? (prix = props.Etude.prix_exams.find(
                  (p) => p.shift_id == shift.IDShift
              ))
            : (prix = false);
        if (prix) {
