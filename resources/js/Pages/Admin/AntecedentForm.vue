        ...
        <form @submit.prevent="submit">
            <div class="form-group mb-4">
                <label
                    ...
                    :class="{ 'text-red-600': errors.libelleAntecedent }"
                    >Libelle Antecedent</label
                >
                <input
                    type="text"
                    id="libelle"
                    v-model="antecedent.libelleAntecedent"
                    class="input input-bordered w-full"
                    :class="{ 'input-error': errors.libelleAntecedent }"
                />
                <p
                    v-if="errors.libelleAntecedent"
                    class="mt-1 text-sm text-red-600"
                >
                    {{ errors.libelleAntecedent[0] }}
                </p>
            </div>
    ...
    try {
        if (props.antecedent.IDAntecedent) {
            const response = await axios.put(
                `/antecedent/${props.antecedent.IDAntecedent}`,
                props.antecedent
            );
