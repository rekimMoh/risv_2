        ...
        >
            ...
            <div class="p-6 overflow-x-auto bg-base-100 rounded-box shadow">
                <table class="table w-full">
                    ...
                    <tbody>
                        <tr
                            v-for="(Etude, index) in Etudes.data"
                            :key="Etude.IDEtude"
                            class="hover"
                        >
                            <td class="font-bold">{{ Etude.IDEtude }}</td>
                            <td>{{ Etude.libelleEtude }}</td>
                            <td>
                                <div class="badge badge-outline">
                                    {{ Etude.libelleService }}
                                </div>
                            </td>
                            <td class="w-44">
                                <button
...
const updateRaw = (Etude) => {
    const index = props.Etudes.data.findIndex(
        (p) => p.IDEtude == Etude.IDEtude
    );
...
const destroy = async (id) => {
    if (confirm("Voulez-vous supprimer ce Etude ?")) {
        ...
        props.Etudes.data = props.Etudes.data.filter(
            (Etude) => Etude.IDEtude !== id
        );
        props.Etudes.total--;
        props.Etudes.to--;
    }
...
const toggleProduct = (index) => {
    ...
    axios
        .post(route("admin.etude.toggle", params))
        .then((response) => {
