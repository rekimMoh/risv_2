        ...
        <form @submit.prevent="submit">
            <div class="form-group mb-4">
                <label
                    ...
                    :class="{ 'text-red-600': errors.libelleShift }"
                    >Libelle Shift</label
                >
                <input
                    type="text"
                    id="libelle"
                    v-model="Shift.libelleShift"
                    class="input input-bordered w-full"
                    :class="{ 'input-error': errors.libelleShift }"
                />
                <p v-if="errors.libelleShift" class="mt-1 text-sm text-red-600">
                    {{ errors.libelleShift[0] }}
                </p>
            </div>
            <div class="form-group mb-4">
                <label
                    for="color"
                    class="block text-sm font-medium text-gray-700 mb-1"
                    :class="{ 'text-red-600': errors.colorShift }"
                    >Couleur Shift</label
                >
                <input
                    type="color"
                    id="color"
                    v-model="Shift.colorShift"
                    class="input input-bordered w-full h-12 p-1"
                    :class="{ 'input-error': errors.colorShift }"
                />
                <p v-if="errors.colorShift" class="mt-1 text-sm text-red-600">
                    {{ errors.colorShift[0] }}
                </p>
            </div>
            <div class="my-20 overflow-x-auto">
                <table class="table w-full">
                    <thead>
                        <tr>
                            <th></th>
                            <th
                                class="text-left px-1.5"
                                v-for="hour in hours"
                                :key="hour.IDhour"
                            >
                                {{ hour.libelleHour }}
                    ...
                    <tbody>
                        <tr v-for="(day, indexDay) in days" :key="indexDay">
                            ...
                            <td
                                v-for="(plan, indexPlan) in plan[indexDay]"
                                :key="indexPlan"
                                :style="{
                                ...
                                class="border border-base-200 cursor-pointer hover:bg-base-200"
                            ></td>
                        </tr>
                    </tbody>
                </table>
...
async function initDayHour() {
    try {
        ...
        if (props.Shift.IDShift && plan.value.length > 0) {
            axios
                .get(`/get-day-hour-shift/${props.Shift.IDShift}`)
                .then((response) => {
                    days.value.forEach((day, indexDay) => {
                        hours.value.forEach((hour, indexHour) => {
                            response.data.forEach((shift) => {
                                if (
                                    shift.day_id == day.IDDay &&
                                    shift.hour_id == hour.IDHour
                                ) {
                                    plan.value[indexDay][
                                        indexHour
                                    ].isSelected = true;
